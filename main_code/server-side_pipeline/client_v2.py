...
class FixedEdgeScheduler:
    """For joint testing: always send one fixed model-resolution hint."""

    def __init__(self, model_name: str = "yolov8n_640") -> None:
        self.model_name = model_name

    def decide(self, state: ClientState) -> ScheduleDecision:
        return ScheduleDecision(model_name=self.model_name)
...
class EdgeClientJointTest:
    def __init__(
        ...
        self.scheduler = FixedEdgeScheduler(model_name=fixed_model)
    ...
    def run(self) -> None:
        ...
        for frame_path in self.frame_paths:
            ...
            self._print_frame_status(frame_id, event, active_tracks, failures)
    ...
    def _print_frame_status(self, frame_id: int, event: str, active_tracks: List[Dict[str, Any]], failures: int) -> None:
        if event == "offload" or failures > 0 or frame_id % max(1, self.target_fps) == 0:
            print(
                f"[FRAME] {frame_id:05d} event={event:<7} "
                f"tracking_age={self.state.tracking_age:<3} "
                f"trackers={len(active_tracks):<3} failures={failures:<2} "
