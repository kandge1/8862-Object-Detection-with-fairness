# Baseline2: fixed yolo26m inference, arrival-order greedy batching, est from client
import json
import time
import asyncio
import heapq
...
BASELINE_FIXED_MODEL_NAME = os.environ.get("BASELINE_FIXED_MODEL_NAME", "yolo26m")
BASELINE_FIXED_IMGSZ = int(os.environ.get("BASELINE_FIXED_IMGSZ", "640"))
...
def build_batch(queue: PriorityRequestQueue) -> List[InferenceRequest]:
    """
    Baseline2 batch builder.

    The first request is still the earliest-arrived request from the priority queue.
    After that, candidates are tried in arrival order using queue.pop().
    It does not prioritize requests with the same opt_model.

    The accept/reject criterion is kept from v5:
        score = avg_map + Jain fairness
    If adding a request improves the score, keep it; otherwise put it back and stop.
    """
    ...
    while not queue.is_empty():
        # Scheme B: no same-opt_model preference; try the next earliest request.
        candidate = queue.pop()
...
async def batch_processing_worker():
    ...
    print(f"Inference backend: model_pool={list(model_pool.models.keys())}, device={DEVICE}, baseline_fixed_model={BASELINE_FIXED_MODEL_NAME}, baseline_fixed_imgsz={BASELINE_FIXED_IMGSZ}")
    ...
    while True:
        if not global_queue.is_empty():
            ...
            # Baseline2: fixed inference backend for every batch/request.
            batch_opt_model = BASELINE_FIXED_MODEL_NAME
            actual_model_name = model_pool.resolve_model_name(BASELINE_FIXED_MODEL_NAME)
            actual_imgsz = BASELINE_FIXED_IMGSZ
...
def health():
    return {
        ...
        "baseline_fixed_model_name": BASELINE_FIXED_MODEL_NAME,
        "baseline_fixed_imgsz": BASELINE_FIXED_IMGSZ,
        "device": DEVICE,
        "conf_threshold": CONF_THRESHOLD,
    }
